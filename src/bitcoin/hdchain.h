...
public:
    ...
    uint256 GetSeedHash();
    void DeriveChildExtKey(uint32_t nCoinType, uint32_t nAccountIndex, bool fInternal, uint32_t nChildIndex, CExtKey& extKeyRet);

    void AddAccount(uint32_t coinType);
    bool GetAccount(uint32_t coinType, uint32_t nAccountIndex, CHDAccount& hdAccountRet);
...
public:
    ...
    CHDPubKey() : nVersion(CHDPubKey::CURRENT_VERSION), nCoinType(0), nAccountIndex(0), nChangeIndex(0) {}
